...
int i; // also a temporary loop value
...
void flamboyantwater() { // main water script
  ...
  while(x < 20){ // repeat for 10 seconds
    temp = SensorValue[S4];
    displayTextLine(3, "temp=%d", temp); // display the temperature
    delay(500);
...
task main(){
 for (i=0;i<2;i++) {
  ...
  // forward until wall using sonar
	...
	while(SensorValue[S4] > 27){
    straightaway(35, 1, false);
  ...
  motor[motorC] = 0;
  // flamboyantwater
  straightaway(50, 150, false);
  flamboyantwater();
  // backwards a little
  straightaway(50, 200, true);
  // turn right
  turn("r", 50, 90, "wood");
  // forward until wall no sonar
  straightaway(50, 1250, false);
  // turn left
  turn("l", 50, 90, "wood");
  // straight
  straightaway(50, 1800, false);
  // turn right
  turn("r", 50, 90, "wood");
